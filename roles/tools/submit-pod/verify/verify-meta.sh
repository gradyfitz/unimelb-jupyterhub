...
VERIFICATION_INPUT_PIPE="$JOBID-prepared"
...
then
    ...
    then
        ...
        then
            ...
            "$VERIFICATION_SCRIPT_LOCATION" "$SUBMISSION_FOLDER" "$VERIFICATION_OUTPUT_FOLDER" "$USERNAME" "$VERIFICATION_DATA_FOLDER" >> "$VERIFICATION_META_OUTPUT_FILE_TEMP"
            VERIFICATION_RESULT=$?
        else
