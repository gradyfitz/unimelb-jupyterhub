...
#   submission.
# SUBMISSION_EPH_FOLDER - From environment, the location which will be used for the student's submission processing while the test runs.
...
#   Configurable Settings.
SETUP_CONFIG_FILE_LOCATION="$ASSIGNMENT_SETUP_FOLDER""/""$SETUP_CONFIG_FILE_NAME"


# Names of each folder, principally arbitrary, but kind to keep them matching
#   their destinations.
VERIFICATION_FOLDER_STRING="verify"
TESTING_FOLDER_STRING="test"
RESULTS_FOLDER_STRING="results"
FINALISE_FOLDER_STRING="finalise"

SOURCE_VERIFICATION_DATA_FOLDER="$ASSIGNMENT_SETUP_FOLDER""/""$ASSIGNMENT_NAME""/""$VERIFICATION_FOLDER_STRING"
SOURCE_TESTING_DATA_FOLDER="$ASSIGNMENT_SETUP_FOLDER""/""$ASSIGNMENT_NAME""/""$TESTING_FOLDER_STRING"
SOURCE_RESULTS_DATA_FOLDER="$ASSIGNMENT_SETUP_FOLDER""/""$ASSIGNMENT_NAME""/""$RESULTS_FOLDER_STRING"
SOURCE_FINALISE_DATA_FOLDER="$ASSIGNMENT_SETUP_FOLDER""/""$ASSIGNMENT_NAME""/""$FINALISE_FOLDER_STRING"
...
then
    ...
    #find "$SOURCE_VERIFICATION_DATA_FOLDER" -exec cp -r "{}" "$VERIFICATION_DATA_FOLDER" \;
    cp -r "$SOURCE_VERIFICATION_DATA_FOLDER"/* "$VERIFICATION_DATA_FOLDER"
fi

if [[ -d "$SOURCE_TESTING_DATA_FOLDER" ]]
then
    echo "[$(date +'%Y/%m/%d %H:%M:%S')]" "Setup - Using custom testing script."
    #find "$SOURCE_TESTING_DATA_FOLDER" -exec cp -r "{}" "$TESTING_DATA_FOLDER" \;
    cp -r "$SOURCE_TESTING_DATA_FOLDER"/* "$TESTING_DATA_FOLDER"
fi

if [[ -d "$SOURCE_RESULTS_DATA_FOLDER" ]]
then
    echo "[$(date +'%Y/%m/%d %H:%M:%S')]" "Setup - Using custom results script."
    #find "$SOURCE_RESULTS_DATA_FOLDER" -exec cp -r "{}" "$RESULTS_DATA_FOLDER" \;
    cp -r "$SOURCE_RESULTS_DATA_FOLDER"/* "$RESULTS_DATA_FOLDER"
fi

if [[ -d "$SOURCE_FINALISE_DATA_FOLDER" ]]
then
    echo "[$(date +'%Y/%m/%d %H:%M:%S')]" "Setup - Using custom finalise script."
    #find "$SOURCE_FINALISE_DATA_FOLDER" -exec cp -r "{}" "$FINALISE_DATA_FOLDER" \;
    cp -r "$SOURCE_FINALISE_DATA_FOLDER"/* "$FINALISE_DATA_FOLDER"
fi
...
then
    ...
    SETUP_RESULT="$?"
    cp "$RAW_SUBMISSIONS_FOLDER""/""$ASSIGNMENT_NAME""/""$USERNAME""/""$(basename "$SUBMITTED_FILENAME")" "$SUBMISSION_EPH_FOLDER"
    if [[ "$SETUP_RESULT" == "0" ]]
    then
        SERVER_CHECKSUM=$(sha1sum "$RAW_SUBMISSIONS_FOLDER""/""$ASSIGNMENT_NAME""/""$USERNAME""/""$(basename "$SUBMITTED_FILENAME")" | head -n 1 | awk '{ print $1 }' | tr -d '\n')
    else
...
then
    ...
    SUBMISSION_VERIFICATION_CERTIFICATE=$(echo "$SUBMISSION_VERIFICATION_PLAINTEXT" | openssl dgst -binary -sha256 -sign "$PRIVATE_SIGNING_KEY" | openssl base64 | tr -d '\n')
fi
...
if [[ -f "$SETUP_META_OUTPUT_FILE_TEMP" ]]
then
